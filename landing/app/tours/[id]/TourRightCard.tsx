...
export default function TourRightCard({ tour, isFull, remaining }: Props) {
  ...
  // Backend, bir turun ilk ek tarihi eklendiği anda zorunlu Başlangıç/Bitiş
  // Tarihi'ni de gerçek bir TourDate kaydına dönüştürüyor — yani tour.dates
  // burada zaten TÜM seçenekleri (zorunlu + ek) içeriyor, ayrıca birleştirmeye
  // gerek yok. Hiç ek tarih eklenmemiş turlarda (mevcut turların tamamı)
  // dates boş kalır: seçici gösterilmez, tour_date_id hiç gönderilmez.
  const allOptions: TourDateOption[] = tour.dates ?? [];
            ...
            allOptions.map((d) => {
              ...
              return (
                <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '11px 0' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.6rem', color: '#BBBBBB', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>
                      {tt.quota} · {fmtDateRange(d, lang === 'en' ? 'en-US' : 'tr-TR')}
                    </div>
                    ...
                    <span style={{ color: dIsFull ? '#EF4444' : '#FF5533', fontWeight: 700, fontSize: '0.88rem' }}>
                      {dIsFull ? tt.full : tt.spotsLeft(dRemaining)}
                    </span>
                  </div>
                </div>
              );
